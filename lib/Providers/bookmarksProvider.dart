...
import 'package:realeestateapp/Data/carouselsliderList.dart';
 ...
 class BookmarksNotifire extends  StateNotifier <List<Item>> {
  ...
  bool toogleBookmark(Item item){
    ...
    if(ItemIsBookmarked){
      state = state.where((bitem)=> bitem.id!= item.id).toList();//bitem is not the same but just to clear any conflex that can be happend 
   return false;
    }
    else{
      state = [...state , item ];
...
final bookmarksProvider =  StateNotifierProvider<BookmarksNotifire,List<Item>>((ref){
